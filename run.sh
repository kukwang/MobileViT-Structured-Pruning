...
for pr in 0.11
do
    pruned_path=/home/user/kwangsoo/paper_codes/my_paper/KICS-2023-mine/save/mobilevit_${mode}_${dataset_name}_${ep}ep_pr${pr}_cosdecay_real.pth
    finetuning_path=/home/user/kwangsoo/paper_codes/my_paper/KICS-2023-mine/save/mobilevit_${mode}_${dataset_name}_${ep}ep_pr${pr}_finetuning_cosdecay_real.pth

    # 1 gpu mobilevit cifar10 training
    CUDA_VISIBLE_DEVICES=0 python3 training.py ${dataset_path} --classes ${classes} --dataset-name ${dataset_name} \
    --model-config ${config} --mode ${mode} --resize ${resize} --epoch ${ep} --train-ratio ${train_ratio} \
    --save-path ${dense_path} --test \
    | tee log_${mode}_${resize}_${ep}ep_pr${pr}_cosdecay_real.txt

    mv log_${mode}_${resize}_${ep}ep_pr${pr}_cosdecay_real.txt ./logs

    # | tee log_${mode}_${resize}_${ep}ep_pr${pr}_cosdecay_real_test.txt

    # mv log_${mode}_${resize}_${ep}ep_pr${pr}_cosdecay_real_test.txt ./logs
    ...
    CUDA_VISIBLE_DEVICES=0 python3 finetuning.py ${dataset_path} --classes ${classes} --dataset-name ${dataset_name} \
    --model-config ${config} --mode ${mode} --resize ${resize} --fprune-rate ${pr} --epoch ${fep} --train-ratio ${train_ratio} \
    --dense-model ${dense_path} --pruned-model ${pruned_path} --save-path ${finetuning_path} \
    | tee log_${mode}_${resize}_${fep}_pr${pr}_finetuning_cosdecay_real.txt

    mv log_${mode}_${resize}_${fep}_pr${pr}_finetuning_cosdecay_real.txt ./logs
